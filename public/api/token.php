...
$origin = trim((string) ($_SERVER['HTTP_ORIGIN'] ?? '')); $referer = trim((string) ($_SERVER['HTTP_REFERER'] ?? '')); $same_site = $origin === '' && (bool) preg_match('#^https://(www\.)?steepstep\.com(?:/|$)#i', $referer); if (!in_array($origin, ['https://steepstep.com', 'https://www.steepstep.com'], true) && !$same_site) { http_response_code(403); exit; } if ($origin !== '') header('Access-Control-Allow-Origin: ' . $origin);
