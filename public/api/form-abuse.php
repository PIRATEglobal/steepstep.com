...
function ss_origin_allowed(string $origin, string $referer): bool {
    if (getenv('STEEPSTEP_LOCAL_DEV') === '1') {
        $port = preg_quote(trim((string) (getenv('STEEPSTEP_LOCAL_ORIGIN_PORT') ?: '4322')), '#');
        return $origin === '' || (bool) preg_match('#^https?://127\.0\.0\.1:' . $port . '(?:/|$)#i', $origin);
    }
    $same_site = $origin === '' && (bool) preg_match('#^https://(www\.)?steepstep\.com(?:/|$)#i', $referer);
    return in_array($origin, ['https://steepstep.com', 'https://www.steepstep.com'], true) || $same_site;
}
