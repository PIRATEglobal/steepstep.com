...
function ss_json(int $status, string $message): never { http_response_code($status); echo json_encode(['status' => $status < 400 ? 'success' : 'error', 'message' => $message], JSON_UNESCAPED_UNICODE); exit; }
...
function ss_rate(string $scope, string $identity, int $max, int $window, string $secret): bool {
    ...
    $times = is_array($times) ? array_values(array_filter($times, static fn($t): bool => is_int($t) && $now - $t < $window)) : [];
